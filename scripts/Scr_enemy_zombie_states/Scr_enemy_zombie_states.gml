...
function enemy_zombie_pursuing(enemy, target) {
	...
	if distance_to_object(target) > detection_circle_max_distance && distance_to_object(target) > detection_cone_length_max && detection_total_threshold <= 0 && Enemy_state != Enemy_state.idle && Enemy_state != Enemy_state.wandering{ //to searching
		home_x = x;
		home_y = y;
		Enemy_state = Enemy_state.idle;
	}
...
function enemy_zombie_detection(enemy) { //detection
